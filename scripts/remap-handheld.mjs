// scripts/remap-handheld.mjs
//
// The "Handheld push/" subfolder holds 46 raw hf_*.mp4 files that were never
...
//
//   node --env-file=.env scripts/remap-handheld.mjs            <- DRY RUN
//   node --env-file=.env scripts/remap-handheld.mjs --confirm  <- actually copy
//
// Copies (does not move) — the originals in "Handheld push/" are left untouched
...
const PREFIX = 'Handheld push/';
const DEST_BASE = 'tag-cameramove-handheld-push-';
...
if (!CONFIRM) {
  ...
  console.log(`  node --env-file=.env scripts/remap-handheld.mjs --confirm\n`);
  process.exit(0);
}
...
for (const { src, dest } of plan) {
  ...
  try {
    // CopySource must be encoded per path segment so the space in "Handheld push/"
