// scripts/remap-whip.mjs
//
// The 10 whip-pan clips are already correctly named
// (tag-cameramove-whip-pan-N.mp4) but sit inside a "whip-pan/" subfolder, which
...
//
//   node --env-file=.env scripts/remap-whip.mjs            <- DRY RUN
//   node --env-file=.env scripts/remap-whip.mjs --confirm  <- copy to root
//
// Copies (does not move); originals in "whip-pan/" are left untouched.
...
if (!CONFIRM) {
  ...
  console.log(`Re-run to copy: node --env-file=.env scripts/remap-whip.mjs --confirm\n`);
  process.exit(0);
}
