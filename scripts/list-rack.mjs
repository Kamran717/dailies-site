...
for (const p of ['Rack focus/', 'rack-focus/', 'Rack Focus/', 'rack focus/']) {
  const k = await count(p);
  console.log(`"${p}" -> ${k.length} mp4`);
  k.slice(0,12).forEach(x => console.log('   ' + x));
}
const root = await count('');
const hits = root.filter(k => /rack/i.test(k));
console.log(`any key containing "rack": ${hits.length}`);
