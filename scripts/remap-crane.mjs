// scripts/remap-crane.mjs
//
// The "crane-rise/" subfolder holds 33 raw hf_*.mp4 files that were never
...
//
//   node --env-file=.env scripts/remap-crane.mjs            <- DRY RUN
//   node --env-file=.env scripts/remap-crane.mjs --confirm  <- actually copy
//
// Copies (does not move) — the originals in "crane-rise/" are left untouched
...
const PREFIX = 'crane-rise/';
const DEST_BASE = 'tag-cameramove-crane-rise-';
...
if (!CONFIRM) {
  ...
  console.log(`  node --env-file=.env scripts/remap-crane.mjs --confirm\n`);
  process.exit(0);
}
...
for (const { src, dest } of plan) {
  ...
  try {
    // CopySource must be encoded per path segment so the space in "crane-rise/"
