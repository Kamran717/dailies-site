...
async function resolvePosterUrl(clipId) {
  ...
  for (const name of [`${clipId}-v1.jpg`, `${clipId}.jpg`]) {
    const url = `${MEDIA_BASE}/posters/${name}`;
    try {
...
export async function castVideo({ faceBase64, scenePrompt, clipId, cameraMove }) {
  ...
  const editInput = posterUrl
    ? {
        prompt:
          'The first image is the exact scene frame from a film. ' +
          'Replace the primary subject in it with the person from the second image. ' +
          'Change NOTHING else: keep the composition, framing, background, other people, ' +
          'wardrobe style, lighting, film grain, lens character, and color grade exactly as they are. ' +
          'Preserve the new person’s facial identity, skin tone, and hair exactly.\n\n' +
