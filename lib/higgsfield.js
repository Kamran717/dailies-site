...
// Motion-exact engine: replaces the person IN the original clip, reusing its
// camera path frame-for-frame instead of re-generating motion from text.
// ~$0.30 (480p) / ~$0.45 (580p) / ~$0.60 (720p) per 5s 24fps cast.
// CAST_ENGINE=legacy in the environment disables it without a deploy.
const WAN_REPLACE_MODEL = 'fal-ai/wan/v2.2-14b/animate/replace';
const WAN_RESOLUTION = process.env.CAST_RESOLUTION || '580p';
const CAST_ENGINE = process.env.CAST_ENGINE || 'wan';
...
/**
 * The original clip's full video, used as the motion/scene reference for the
 * Wan Animate Replace engine. Same naming convention as the posters.
 */
async function resolveClipVideoUrl(clipId) {
  if (!clipId || !/^[a-z0-9-]{1,120}$/i.test(clipId)) return null;
  for (const name of [`${clipId}-v1.mp4`, `${clipId}.mp4`]) {
...
export async function castVideo({ faceBase64, scenePrompt, clipId, cameraMove }) {
  ...
  // 1. Face -> fal storage (poster + clip-video lookups run concurrently)
  const [faceUrl, posterUrl, clipVideoUrl] = await Promise.all([
    fal.storage.upload(blobFromBase64(faceBase64)),
    resolvePosterUrl(clipId),
    CAST_ENGINE === 'wan' ? resolveClipVideoUrl(clipId) : Promise.resolve(null),
  ]);

  // 1b. Motion-exact path: swap the person inside the ORIGINAL clip. The
  //     camera path, scene and timing are the original's, frame for frame.
  //     Any failure falls through to the legacy generate-from-still path.
  if (clipVideoUrl) {
    try {
      const wan = await fal.subscribe(WAN_REPLACE_MODEL, {
        input: {
      ...
      const wanVideoUrl = wan?.data?.video?.url;
      if (wanVideoUrl) {
        // Share still: composite into the HD poster when one exists; if that
        // fails, the cast itself still succeeds with the poster as the still.
        let sceneUrl = posterUrl;
        try {
          const still = await fal.subscribe(EDIT_MODEL, {
            input: {
              prompt:
                'FACE AND IDENTITY SWAP. Image 1 is a film frame; image 2 is a photo of a person. ' +
                'Replace the main subject in image 1 with the person from image 2, clearly showing their ' +
                'face, skin tone, and hairstyle, matching the pose, lighting, and color grade. ' +
                'Keep everything else unchanged.',
              image_urls: [posterUrl || clipVideoUrl, faceUrl],
              num_images: 1,
              output_format: 'jpeg',
            },
            logs: false,
          });
          sceneUrl = still?.data?.images?.[0]?.url || sceneUrl;
        } catch (err) {
          console.error('[cast] share-still composite failed (non-fatal)', err?.message || err);
        }
        return { videoUrl: wanVideoUrl, sceneUrl };
      }
      console.error('[cast] wan replace returned no video; falling back to legacy');
    } catch (err) {
      console.error('[cast] wan replace failed; falling back to legacy', err?.message || err);
    }
  }
